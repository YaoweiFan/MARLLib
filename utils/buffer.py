...
def get_num_items(indexing_item, max_size):
...
class EpisodeBatch:
        ...
        for field_key, field_info in scheme.items():
            ...
            if episode_const:
                ...
                if field_key is 'policy':
                    self.data.transition_data[field_key] = th.ones((batch_size, max_seq_length, *shape),
    ...
    def update(self, data, bs=slice(None), ts=slice(None), mark_filled=True):
        ...
        for k, v in data.items():
            ...
            check_safe_view(v, target[k][_slices])
    ...
    def __getitem__(self, item):
        ...
        if isinstance(item, str):
            ...
            ret = EpisodeBatch(new_scheme, new_groups, self.batch_size, self.max_seq_length,
                               data=new_data, device=self.device)
            return ret
        else:
            item = parse_slices(item)
            new_data = new_data_sn()
            for k, v in self.data.transition_data.items():
            ...
            ret_bs = get_num_items(item[0], self.batch_size)
            ret_max_t = get_num_items(item[1], self.max_seq_length)
...
class ReplayBuffer(EpisodeBatch):
    def __init__(self, scheme, groups, buffer_size, max_seq_length, preprocess=None, device="cpu"):
        super(ReplayBuffer, self).__init__(scheme, groups, buffer_size, max_seq_length,
...
class BestExperienceBuffer(EpisodeBatch):
    def __init__(self, scheme, groups, buffer_size, max_seq_length, preprocess=None, device="cpu"):
        super(BestExperienceBuffer, self).__init__(scheme, groups, buffer_size, max_seq_length, preprocess=preprocess,
                                                   device=device)
